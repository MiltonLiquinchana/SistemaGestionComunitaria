...
function inicio() {
    ...
    if (ref_genera_input[5].value !== "") {/*comprobamos que el input que temporalmente tomara el valor de la opcion seleccionada no este vacio*/
        ref_genera_select[0].value = ref_genera_input[5].value;/*de ser el caso asignamos lo que tenga el input al select esto con el proposito de que
         * no se cambie el valor al recargar la pagina*/
    }
    if (ref_genera_input[6].value !== "") {
        ref_genera_select[1].value = ref_genera_input[6].value;
    }
...
function listarConsumos() {
    if (ref_genera_select[0].value > 0) {
        window.location.replace('Controles?menu=PAgua&accion=buscarconsumoimpaga&dato=' + ref_genera_input[3].value + '&pk_medidor=' + ref_genera_select[0].value);
    }
}
